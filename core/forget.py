...
def forget(subject: str) -> dict:
    """Erase `subject` from memory. Returns a measured receipt."""
    with store.connect() as conn:
        ...
        with conn.transaction():
            with conn.cursor() as cur:
                ...
                if exclusive:
                    cur.execute(
                        "DELETE FROM edges WHERE source_id = ANY(%s) OR target_id = ANY(%s)",
                        (exclusive, exclusive),
                    )
                ...
                cur.execute("DELETE FROM documents WHERE subject = %s", (subject,))
                receipt["docs"] = cur.rowcount

                # crypto-shred: destroy the subject's data key
                store.crypto_shred(conn, subject)

                # measured, tamper-evident erasure event
                cur.execute(
                    """
                    INSERT INTO erasure_events
                      (subject, t_before, docs_removed, nodes_removed, edges_removed, nodes_invalidated)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING id
                    """,
                    (subject, t_before, receipt["docs"], receipt["nodes"],
                     receipt["edges"], receipt["invalidated"]),
                )
                event_id = cur.fetchone()[0]

                cur.execute(
                    "INSERT INTO timeline (kind, subject, detail) VALUES ('forget', %s, %s)",
                    (subject,
    ...
    return {"subject": subject, "t_before": t_before, "event_id": str(event_id), **receipt}


def prior_state(subject: str, t_before: str) -> list[dict]:
...
def verify_gone(subject: str) -> dict:
    """Proof-of-absence: after erasure, the subject's exclusive knowledge and key are gone."""
    with store.connect() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT count(*) FROM nodes WHERE %s::STRING = ANY(subjects) AND deleted_at IS NULL", (subject,)
        )
        live_nodes = cur.fetchone()[0]
        cur.execute("SELECT count(*) FROM documents WHERE subject = %s", (subject,))
        live_docs = cur.fetchone()[0]
        cur.execute(
            "SELECT wrapped_dek IS NULL, destroyed_at IS NOT NULL FROM subject_keys WHERE subject = %s",
            (subject,),
        )
        row = cur.fetchone()
