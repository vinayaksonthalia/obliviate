...
- [ ] **Functional demo app URL** — deployed on AWS (EC2 + Lambda + S3), reachable by judges.
...
- [x] **≥ 2 CockroachDB tools, meaningfully integrated** — we use **four**:
  1. **Managed MCP Server** — the agent reads/writes memory through the Cloud MCP server.
  ...
  4. **Row-level TTL** — retention/decay enforcement.
  - Plus the **`ccloud` CLI** for provisioning/reset (documented touchpoint).
- [x] **≥ 1 AWS service** — **AWS Lambda** (certificate signing) + **Amazon S3** (object-locked
      erasure certificates) + **EC2** (hosting the deployed app).
